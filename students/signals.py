...
def reassign_primary_guardian(sender, instance, **kwargs):
    ...
    try:
        with transaction.atomic():
            next_guardian = StudentGuardian.objects.select_for_update().filter(
            ...
            if next_guardian:
                ...
                logger.info(
                    ...
                    f"to guardian {next_guardian.guardian_id}"
                )
    except Exception:
        logger.error(
            f"Failed to reassign primary guardian for student {instance.student_id}",
            exc_info=True
        )
